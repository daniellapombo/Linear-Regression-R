...
dmodel_fit = lm(ViolentCrimesPerPop ~PctFam2Par + raceCatWhite + NumStreet + 
                  state + pctUrban + PctUnemployed + MedRent + CatPopUnderPov + 
...
#Constant variance and linearity assumption tests
par(mfrow = c(1,1))
plot(dmodel_fit$residuals~predict(dmodel_fit))
rstudent = rstudent(dmodel_fit)
plot(rstudent~predict(dmodel_fit))
abline(-2,0)
abline(2,0)
...
#Constant variance and linearity assumption tests
par(mfrow = c(1,1))
plot(d1model_fit$residuals~predict(d1model_fit))
rstudent = rstudent(d1model_fit)
plot(rstudent~predict(d1model_fit))
abline(-2,0)
abline(2,0)

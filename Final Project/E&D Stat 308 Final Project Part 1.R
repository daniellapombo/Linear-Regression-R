...
X = seq(min(com_dat1$PctUnemployed), max(com_dat1$PctUnemployed))
Ypredict = unempl_fit$coefficients[1] + unempl_fit$coefficients[2]*X
ci = confintline(com_dat1$PctUnemployed, sdUnempl, Ypredict, n, meanUnempl, RMSE, 0.95)
plot(com_dat1$ViolentCrimesPerPop~com_dat1$PctUnemployed)

lines(com_dat1$PctUnemployed, ci[,"CILB"],lty = "dashed", col = "red")
lines(com_dat1$PctUnemployed, ci[,"CIUB"],lty = "dashed", col = "red")
